...
public:
  ...
  void prepareToPlay(double sampleRate,
    ...
    oversampler.initProcessing(static_cast<size_t>(samplesPerBlockExpected));
    oversampler.reset();
    updateFilters();
    preClipFilter.reset();
  ...
  void process(juce::AudioBuffer<float> &buffer) override {
    ...
    // Step 1: Wrap buffer in an AudioBlock and perform upsampling (8x
    // oversampling) This applies an anti-imaging filter before processing at 8x
    // sample rate.
    juce::dsp::AudioBlock<float> inputBlock(buffer);
    juce::dsp::AudioBlock<float> oversampledBlock =
        oversampler.processSamplesUp(inputBlock);

    const size_t numChannels = oversampledBlock.getNumChannels();
    const size_t numSamples = oversampledBlock.getNumSamples();

    preClipFilter.ensureNumChannels(static_cast<int>(numChannels));
    postClipFilter.ensureNumChannels(static_cast<int>(numChannels));
    dcBlocker.ensureNumChannels(static_cast<int>(numChannels));

    // Step 2: Perform non-linear overdrive processing at the oversampled rate
    // (e.g. 352.8 kHz). At this high sample rate, harmonics generated by
    // std::tanh extend far above 20 kHz without folding back (aliasing) into
    // the audible range!
    for (size_t ch = 0; ch < numChannels; ++ch) {
      ...
      for (size_t i = 0; i < numSamples; ++i) {
        ...
        x = preClipFilter.processSample(static_cast<int>(ch), x);
        ...
        x = postClipFilter.processSample(static_cast<int>(ch), x);

        // Step E: DC Blocking Filter (~10 Hz)
        // Models C9 (0.047uF) & C10 (1uF) coupling caps removing DC offset
        x = dcBlocker.processSample(static_cast<int>(ch), x);
    ...
    // The oversampler applies a steep anti-aliasing half-band low-pass filter
    // that cuts ALL frequency components exceeding the original Nyquist
    // frequency (exceeding human audible range) before downsampling!
    oversampler.processSamplesDown(inputBlock);

    // Step 4: Apply Output Level Control on the downsampled buffer
...
private:
  void updateFilters() {
    // Calculate filter coefficients at the oversampled sample rate (e.g. 352.8
    // kHz) so that frequency response curves remain accurate during oversampled
    // processing.
    const float oversampledRate =
        currentSampleRate *
        static_cast<float>(oversampler.getOversamplingFactor());
  ...
  // 8x oversampling (factor = 3 -> 2^3 = 8) using high-quality Polyphase IIR
  // half-band filters
